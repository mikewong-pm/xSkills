...
MCP_ID=$(grep -r "ant_token_analytics" ~/.claude/sessions/*.json 2>/dev/null | \
  grep -o 'mcp__[a-z0-9-]*__ant_token_analytics' | head -1 | \
  sed 's/mcp__//;s/__ant_token_analytics//' 2>/dev/null || true)
...
if [ -n "$MCP_ID" ]; then
  ...
  echo "   2. 找到工具名格式为: mcp__<UUID>__ant_token_analytics"
  echo "   3. 复制其中的 <UUID> 部分（如 3211031c-c61f-4f99-8441-0878d032a450）"
  echo ""
...
echo "   /whale-control SIREN bsc"
echo "   /whale-control DEXE ethereum"
echo ""
