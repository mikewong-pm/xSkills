...
**数据整理:** 将 Top 50 分类:
1. **销毁/零地址**: 0x0000...dead 等 → 从有效供应中扣除
2. **交易所地址**: 带 exchange 标签 → 单独统计
3. **合约地址**: LP 池、锁仓、质押合约 → 单独统计
4. **普通地址 (EOA)**: 聚类分析目标
...
1. 将每个地址的 related_wallets 与 Top 50 Holders 做交集
2. Holder A 的关联钱包包含 Holder B → A、B 归为同一 Cluster
3. 递归合并: A→B、B→C → A、B、C 同一 Cluster
4. 每个 Cluster 计算: 地址数、合计持仓量、占有效供应量百分比
...
- `related_wallets` 全部无关联 → 跳过聚类，仅输出原始 Top Holder 集中度
- `labels` 无法归因 → 标注"未知实体"
- 代币无合约交易对 → 跳过 Phase 4
